...
const MANAGED_STATUSES = ["awaiting_payment", "awaiting_dropoff", "item_received", "awaiting_collection", "item_released"];
        ...
        {loading ? (
          ...
          <section className="transactions-empty-state">
            <EmptyState
              ...
              description="Try broadening the date range, clearing a chip, or searching with a different seller, buyer, or item name."
            />
          </section>
        ) : (
          <>
              ...
              <thead>
                <tr>
                  <th scope="col" className="transactions-table__checkbox-col">
                    <input
                      type="checkbox"
                      aria-label={allVisibleSelected ? "Deselect current page rows" : "Select current page rows"}
                  ...
                  <th scope="col">
                    <button type="button" className="transactions-table__header-btn" onClick={() => handleSort("createdAt")}>
                      Transaction <span className="transactions-table__sort">{sortConfig.key === "createdAt" ? (sortConfig.direction === "asc" ? "↑" : "↓") : "↕"}</span>
                    </button>
                  </th>
                  <th scope="col">
                    <button type="button" className="transactions-table__header-btn" onClick={() => handleSort("item")}>
                      Item <span className="transactions-table__sort">{sortConfig.key === "item" ? (sortConfig.direction === "asc" ? "↑" : "↓") : "↕"}</span>
                    </button>
                  </th>
                  <th scope="col">
                    <section className="transactions-table__header-stack">
                      <button type="button" className="transactions-table__header-btn" onClick={() => handleSort("seller")}>
                        Seller <span className="transactions-table__sort">{sortConfig.key === "seller" ? (sortConfig.direction === "asc" ? "↑" : "↓") : "↕"}</span>
                      </button>
                      <label className="transactions-table__header-filter">
                        <Icon name="chevron-down" className="transactions-table__header-filter-icon" />
                        <span className="sr-only">Filter by seller</span>
                        <select
                          value={sellerFilter}
                          onChange={(event) => {
                  ...
                  <th scope="col">
                    <section className="transactions-table__header-stack">
                      <button type="button" className="transactions-table__header-btn" onClick={() => handleSort("buyer")}>
                        Buyer <span className="transactions-table__sort">{sortConfig.key === "buyer" ? (sortConfig.direction === "asc" ? "↑" : "↓") : "↕"}</span>
                      </button>
                      <label className="transactions-table__header-filter">
                        <Icon name="chevron-down" className="transactions-table__header-filter-icon" />
                        <span className="sr-only">Filter by buyer</span>
                        <select
                          value={buyerFilter}
                          onChange={(event) => {
                  ...
                  <th scope="col">
                    <button type="button" className="transactions-table__header-btn" onClick={() => handleSort("amount")}>
                      Amount <span className="transactions-table__sort">{sortConfig.key === "amount" ? (sortConfig.direction === "asc" ? "↑" : "↓") : "↕"}</span>
                    </button>
                  </th>
                  <th scope="col">
                    <button type="button" className="transactions-table__header-btn" onClick={() => handleSort("status")}>
                      Status <span className="transactions-table__sort">{sortConfig.key === "status" ? (sortConfig.direction === "asc" ? "↑" : "↓") : "↕"}</span>
                    </button>
                  </th>
                  <th scope="col" className="transactions-table__actions-col">
                    Actions <span className="transactions-table__sort">↕</span>
                  </th>
                </tr>
              </thead>
              <tbody>
                {paginatedTransactions.map((transaction) => {
                  const dropoff = bookingMap[transaction.dropoffId];
                  const collection = bookingMap[transaction.collectionId];
                  const isItemTrade = transaction.transaction_type === "item_trade" || Boolean(transaction.offered_listing_id);
                  const isSelected = validSelectedIds.includes(transaction.id);
                  const isExpanded = expandedRowId === transaction.id;
                  const itemDescription = transaction.itemDescription || (isItemTrade
                    ? `${transaction.requested_item || "Listed item"} for ${transaction.offered_item || "offered item"}`
                    : `${transaction.quantity || 1} item${Number(transaction.quantity || 1) > 1 ? "s" : ""} in this transaction`);

                  return [
                      <tr key={transaction.id} className={`transactions-table__row ${isSelected ? "transactions-table__row--selected" : ""}`}>
                        <td className="transactions-table__checkbox-col">
                          <input
                            type="checkbox"
                          ...
                          <section className="txn-transaction-cell">
                            <section className="txn-id-row">
                              ...
                              <button
                                ...
                                title={copiedId === transaction.id ? "Copied" : "Copy transaction ID"}
                              >
                        ...
                        <td>
                          <section className="txn-item-cell" title={transaction.item}>
                            ...
                            <section className="txn-item-copy">
                              ...
                              <p className="txn-item__meta" title={itemDescription}>{itemDescription}</p>
                            </section>
                          </section>
                        </td>
                        <td>
                          <section className="txn-person">
                            ...
                            <section className="txn-person__copy">
                              <p className="txn-person__name" title={transaction.seller.name}>{transaction.seller.name}</p>
                              <p className="txn-person__id" title={transaction.seller.studentId}>{transaction.seller.studentId}</p>
                            </section>
                          </section>
                        </td>
                        <td>
                          <section className="txn-person">
                            ...
                            <section className="txn-person__copy">
                              <p className="txn-person__name" title={transaction.buyer.name}>{transaction.buyer.name}</p>
                              <p className="txn-person__id" title={transaction.buyer.studentId}>{transaction.buyer.studentId}</p>
                            </section>
                          </section>
                        </td>
                        <td>
                          <section className="txn-amount-cell">
                            <p className="txn-price">
                              {isItemTrade ? "Item trade" : `R ${Number(transaction.totalAmount || 0).toLocaleString("en-ZA")}`}
                            </p>
                            <p className="txn-amount__meta">{getItemTypeLabel(transaction)}</p>
                          </section>
                        </td>
                        <td>
                          <section className="txn-status-stack">
                            <span className={`txn-status-dot txn-status-dot--${getStatusTone(transaction.status)}`} aria-hidden="true" />
                            <StatusBadge status={transaction.status} />
                        ...
                        <td className="transactions-table__actions-col">
                          ...
                          >
                            <button
                              ...
                              aria-label={`Open actions for transaction ${transaction.id}`}
                            ...
                            {openMenuId === transaction.id ? (
                              <section className="txn-actions__menu" role="menu">
                                <button type="button" role="menuitem" onClick={() => toggleExpandedRow(transaction.id)}>View</button>
                                <button type="button" role="menuitem" onClick={() => openStatusEditor(transaction.id)}>Edit</button>
                                <button
                                  type="button"
                                  role="menuitem"
                                  onClick={() => {
                                ...
                                <button
                                  type="button"
                                  role="menuitem"
                                  className="txn-actions__menu-danger"
                                  onClick={() => {
                                    setOpenMenuId("");
                                    onTransactionStatusChange(transaction.id, "cancelled");
                                  }}
                          ...
                          <td colSpan={8}>
                            <section className="transactions-detail-card">
                              <section className="transactions-detail-card__group">
                                <p className="transactions-detail-card__label">Drop-off</p>
                                {dropoff ? (
                                  <p className="transactions-detail-card__value">
                                    {formatDate(dropoff.scheduledDate)} at {dropoff.scheduledTime} · {BOOKING_STATUS_META[dropoff.status]?.label || dropoff.status}
                                  </p>
                                ) : (
                                  <p className="transactions-detail-card__value transactions-detail-card__value--muted">No drop-off booking linked yet.</p>
                                )}
                              </section>
                              <section className="transactions-detail-card__group">
                                <p className="transactions-detail-card__label">Collection</p>
                                {collection ? (
                                  <p className="transactions-detail-card__value">
                                    {formatDate(collection.scheduledDate)} at {collection.scheduledTime} · {BOOKING_STATUS_META[collection.status]?.label || collection.status}
                                  </p>
                                ) : (
                                  <p className="transactions-detail-card__value transactions-detail-card__value--muted">No collection booking linked yet.</p>
                                )}
                              </section>
                              <section className="transactions-detail-card__group">
                                <p className="transactions-detail-card__label">Payment</p>
                                <p className="transactions-detail-card__value">{transaction.paymentMethod || "Not specified"}</p>
                              </section>
                            </section>
                          </td>
                        </tr>
                  ...
                  <section className="txn-item-cell" title={transaction.item}>
                    ...
                    <section className="txn-item-copy">
                      ...
                      <p className="txn-item__meta">
                        {transaction.itemDescription || (isItemTrade ? "Item trade" : "Marketplace sale")}
                      </p>
                    </section>
                  </section>
