      ...
      htmlContent: `
        <div style="margin:0;padding:32px 16px;background:#e3efe6;font-family:Arial,Helvetica,sans-serif;color:#10261a;">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;margin:0 auto;background:#ffffff;border-radius:18px;overflow:hidden;box-shadow:0 14px 40px rgba(15,23,42,0.12);">
            <tr>
              <td style="padding:30px 28px;text-align:center;background:#116b55;">
                <div style="font-size:24px;font-weight:800;letter-spacing:0.4px;color:#ffffff;">CampusXchange</div>
                <div style="display:inline-block;margin-top:14px;padding:7px 12px;border-radius:999px;background:rgba(255,255,255,0.16);color:#dff7e8;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;">Payment Confirmation</div>
              </td>
            </tr>
            <tr>
              <td style="padding:34px 30px 18px;text-align:center;">
                <div style="width:58px;height:58px;line-height:58px;margin:0 auto 16px;border-radius:50%;background:#dcfce7;color:#166534;font-size:32px;font-weight:800;">✓</div>
                <h1 style="margin:0 0 10px;font-size:26px;line-height:1.25;color:#10261a;">Payment Successful</h1>
                <p style="margin:0;color:#4b6356;font-size:15px;line-height:1.6;">Your PayFast sandbox payment for <strong>${safeItem}</strong> has been confirmed.</p>
              </td>
            </tr>
            <tr>
              <td style="padding:12px 30px 4px;text-align:center;">
                <div style="display:inline-block;padding:20px 26px;border-radius:16px;background:#f8fafc;border:1px solid #e2e8f0;">
                  <div style="font-size:34px;line-height:1;font-weight:800;color:#116b55;">${formattedAmount}</div>
                  <div style="margin-top:10px;color:#64748b;font-size:12px;text-transform:uppercase;letter-spacing:0.08em;">Transaction ID</div>
                  <div style="margin-top:4px;color:#10261a;font-size:14px;font-weight:700;">${transactionId}</div>
                </div>
              </td>
            </tr>
            <tr>
              <td style="padding:24px 30px 8px;">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border:1px solid #e2e8f0;border-radius:14px;overflow:hidden;">
                  <tr>
                    <td style="padding:13px 16px;background:#f8fafc;color:#64748b;font-size:13px;font-weight:700;">Item</td>
                    <td style="padding:13px 16px;background:#f8fafc;color:#10261a;font-size:13px;text-align:right;font-weight:700;">${safeItem}</td>
                  </tr>
                  <tr>
                    <td style="padding:13px 16px;color:#64748b;font-size:13px;font-weight:700;border-top:1px solid #e2e8f0;">Amount</td>
                    <td style="padding:13px 16px;color:#10261a;font-size:13px;text-align:right;font-weight:700;border-top:1px solid #e2e8f0;">${formattedAmount}</td>
                  </tr>
                  <tr>
                    <td style="padding:13px 16px;background:#f8fafc;color:#64748b;font-size:13px;font-weight:700;border-top:1px solid #e2e8f0;">Payment Method</td>
                    <td style="padding:13px 16px;background:#f8fafc;color:#10261a;font-size:13px;text-align:right;font-weight:700;border-top:1px solid #e2e8f0;">PayFast Sandbox</td>
                  </tr>
                  <tr>
                    <td style="padding:13px 16px;color:#64748b;font-size:13px;font-weight:700;border-top:1px solid #e2e8f0;">Status</td>
                    <td style="padding:13px 16px;color:#166534;font-size:13px;text-align:right;font-weight:800;border-top:1px solid #e2e8f0;">Confirmed</td>
                  </tr>
                  <tr>
                    <td style="padding:13px 16px;background:#f8fafc;color:#64748b;font-size:13px;font-weight:700;border-top:1px solid #e2e8f0;">Transaction ID</td>
                    <td style="padding:13px 16px;background:#f8fafc;color:#10261a;font-size:13px;text-align:right;font-weight:700;border-top:1px solid #e2e8f0;">${transactionId}</td>
                  </tr>
                </table>
              </td>
            </tr>
            <tr>
              <td style="padding:18px 30px 34px;text-align:center;">
                <p style="margin:0 0 18px;color:#4b6356;font-size:14px;line-height:1.6;">The seller can now book the drop-off slot in My Bookings.</p>
                <a href="${bookingsUrl}" style="display:inline-block;padding:13px 22px;border-radius:12px;background:#116b55;color:#ffffff;text-decoration:none;font-size:14px;font-weight:800;">Go to My Bookings</a>
                <p style="margin:18px 0 0;color:#64748b;font-size:12px;line-height:1.5;">This was a sandbox/demo payment. No real money was transferred.</p>
              </td>
            </tr>
          </table>
        </div>
      `,
      textContent: `Payment successful. Item: ${safeItem}. Amount: ${formattedAmount}. Payment Method: PayFast Sandbox. Status: Confirmed. Transaction ID: ${transactionId}. Go to My Bookings: ${bookingsUrl}. This was a sandbox/demo payment.`,
    }),
  });
...
Deno.serve(async (req) => {
  ...
  const { data: transaction, error } = await supabase
    .from("transactions")
    .select("id, item, listing_id, buyer_id, price, payment_status, transaction_type")
    .eq("id", transactionId)
    .single();
